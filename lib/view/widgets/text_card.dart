...
class ImageCardWidget extends StatelessWidget {
  final TextCompletionData textCompletionData;
  const ImageCardWidget({Key? key, required this.textCompletionData}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return Container(
        decoration: BoxDecoration(
        ...
        child: ListTile(
          leading: textCompletionData.isSendMassage ? const CircleAvatar(backgroundColor: Colors.green,child: Icon(Icons.person,color: Colors.white,),) : const CircleAvatar(child: Icon(Icons.ac_unit,),),
          title: textCompletionData.isSendMassage?
          Image.file(File(textCompletionData.text)):
          Image.network(textCompletionData.text),
        )
    );
  }
}
